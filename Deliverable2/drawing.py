"""""
Created by Iver Rannug Fossan and Oscar Wiersdalen Thunold

Task 1
"""""
import turtle as t

# Rectangle-klassen bygger på eksempelet fra forelesning 8.
# Kilde: Forelesningsnotat (Lecture 8).


# Class pulled from lecture notebook 8
class Rectangle:
    def __init__(self, lower_left, upper_right):
        self._ll = lower_left
        self._width = upper_right[0] - lower_left[0]
        self._height = upper_right[1] - lower_left[1]

    # Method to calculate area of the rectangle
    def area(self):
        return self._width * self._height
    
    # skriver ut venstre nedre og høyre øvre hjørne
    def info(self):
        ur = (self._ll[0] + self._width, self._ll[1] + self._height)
        print(f"Rectangle: lower left {self._ll}, upper right {ur}")
    

    def draw(self, Turtle, scale=50):
        
        Turtle.pensize(3)
        Turtle.speed(3)
        
        Turtle.penup()
        Turtle.goto(self._ll[0]*scale, self._ll[1]*scale)
        Turtle.pendown()
        

        for _ in range(2):
            Turtle.forward(self._width*scale)
            Turtle.left(90)
            Turtle.forward(self._height*scale)
            Turtle.left(90)

if __name__ == "__main__":

    Turtle = t.Turtle()

    # Tre forskjellige rektangler
    rects = [
        Rectangle((-1.5, -2), (1.5, 2)),   # sentrert, bredde=3, høyde=4
        Rectangle((-3, -1), (0, 1)),       # litt til venstre
        Rectangle((0.5, -0.5), (2.5, 1.5)) # litt til høyre
    ]

    for r in rects:
        r.draw(Turtle, scale=50)

    t.done()

    colors = ["red", "green", "orange"]
    for r, c in zip(rects, colors):
        Turtle.pencolor(c)
        r.draw(Turtle, scale=50)
